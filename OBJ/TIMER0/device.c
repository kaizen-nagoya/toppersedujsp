 ...
 * 
 *  @(#) $Id: device.c,v 1.1 2004/02/27 13:56:20 roi Exp $
 */
...
initial_timer()
{
	...
	sil_wrb_mem((VP)TADR_SFR_TA0MR, 0x80);	/* タイマモード	クロック：1/32	*/
											/* タイマ値の初期化				*/
	sil_wrh_mem((VP)TADR_SFR_TA0, TIMER_CLOCK-1);
											/* 割り込みレベルの設定			*/
	sil_wrb_mem((VP)(TADR_SFR_INT_BASE+TADR_SFR_TA0IC_OFFSET), TINTLVL0);
	sil_wrb_mem((VP)TADR_SFR_TABSR, 0x01);	/* カウント開始					*/
}
