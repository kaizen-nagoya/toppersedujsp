 ...
 * 
 *  @(#) $Id: timer3.c,v 1.2 2004/03/15 09:55:24 roi Exp $
 */
...
unsigned char sport = 0xff;

/*
 * ユーザタスク
 * スタブ用に使用、スイッチのシミュレーション
 */
void user_task(VP_INT exif)
{
	T_SERIAL_RPOR rpor;
	char buf[2];

	serial_ctl_por(TASK_PORTID, (IOCTL_CRLF | IOCTL_FCSND | IOCTL_FCRCV));
	syslog(LOG_NOTICE, "Press Key::Start(r),Stop(s),TimeUp(u)");
	for(;;){
		serial_ref_por(TASK_PORTID, &rpor);
		if(rpor.reacnt > 0){
			serial_rea_dat(TASK_PORTID, buf, 1);
			switch(buf[0]){
			case 'r':
			case 'R':
				if(sport & P8_SW1){
					...
					syslog(LOG_NOTICE, "already timer start !!");
				break;
			case 's':
			case 'S':
				if(sport & P8_SW1)
					syslog(LOG_NOTICE, "already timer stop !!");
				else{
					sport |= P8_SW1;
